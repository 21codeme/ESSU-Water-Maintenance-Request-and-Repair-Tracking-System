        ...
        async create(reportData) {
            try {
                ...
                Object.keys(reportData).forEach(key => {
                    ...
                    // Map 'image' key to 'photo' for backend compatibility
                    const fieldName = (key === 'image') ? 'photo' : key;
                    
                    // Always append required fields (even if empty) so backend can validate
                    // For optional fields (like photo), only append if value exists
                    if (requiredFields.includes(key)) {
                        // Always append required fields (backend will validate)
                        formData.append(fieldName, value || '');
                        console.log(`  ${value && value.length > 0 ? '✅' : '⚠️'} Added ${fieldName}: "${value || ''}" ${value && value.length > 0 ? '' : '(EMPTY - will fail validation)'}`);
                    } else if (value !== null && value !== undefined && value !== '') {
                        // For optional fields, only append if value exists
                        formData.append(fieldName, value);
                        console.log(`  ✅ Added ${fieldName}: ${value instanceof File ? `[File] ${value.name}` : `"${value}"`}`);
                    } else {
                        console.log(`  ⏭️ Skipped optional ${fieldName}: ${value === '' ? 'empty string' : value === null ? 'null' : 'undefined'}`);
                    }
                });
