...
const API = {
    ...
    async request(url, options = {}) {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                // This is expected for some error responses
            }
            ...
            if (response.status === 401) {
                ...
                // For "My Reports" page, don't redirect - allow public access
                // Just throw the error so the calling code can handle it (e.g., use public endpoint)
                const currentPath = window.location.pathname;
                if (currentPath.includes('myreports')) {
    ...
    handleAuthError() {
        ...
        // Don't redirect on "My Reports" page - allow public access
        // Only redirect if not on login or myreports pages
        const currentPath = window.location.pathname;
        if (!currentPath.includes('login') && !currentPath.includes('myreports')) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addEventListener('error', function(event) {
        ...
        // Ignore syntax errors from HTML files (often caused by BOM or invisible characters)
        if (event.error && event.error instanceof SyntaxError) {
            if (event.filename && event.filename.includes('index.html')) {
                // Silently ignore syntax errors in HTML files - they're usually harmless
                return;
            }
        }
